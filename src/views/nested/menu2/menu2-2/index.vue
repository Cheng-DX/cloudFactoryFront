    ...
    <el-dialog :visible.sync="dialogFormVisible">
      <el-form ref="form" :model="editForm" label-width="150px">
        <el-form-item label="产品类型ID">
          <el-input v-model="editForm.productTypeId" disabled />
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    updateType() {
      ...
      axios.get('/productType/updateType?name=' + this.editForm.name + '&id=' + this.editForm.productTypeId)
        .then(res => {
    ...
    addType() {
      ...
      axios.get('/productType/addType?name=' + this.addForm.name)
        .then(res => {
          if (res.data.result === 1) {
            this.$message.success(res.data.message)
          } else {
    ...
    deleteFun(row) {
      axios.get('productType/deleteById?id=' + row.productTypeId)
        .then(res => {
          this.fetchData()
          if (res.data.result === 1) {
